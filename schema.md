...
**准入硬门槛**：只收录许可**允许二次分发**的内容；声明「仅个人使用」或「禁止再分发」的源一律不收。

| 分区 | 级别 | 使用范围 | 是否发布 | 标注要求 |
|---|---|---|---|---|
| `main` | C1 可商用 | 任何目的，含商业 | ✅ 公开发布 | 逐曲署名 |
| `piano-special` | C2 不可商用 | 非商业用途 | ✅ 公开发布 | 醒目 NC 标注 |
| `study` | C3 仅供研究/学习 | 研究 / 教学 / 学习 | ✅ 发布 | 用途限制 + 来源声明 |
| `pending` | — | 许可待确认 | ❌ 未发布 | — |

判定依据与逐源审查：`docs/EXPANSION-PLAN-BATCH34.md`
